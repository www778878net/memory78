...
- 知识库也可作为 AI 客服的基础来使用，对商务展示案例也有帮助。
- 一套知识，**人和 AI 都能读**：人翻目录、AI 检索共用同一套结构，不重复维护。
- **向量（语义搜索）**解决"按意思找得准"，**QMD 智能搜索**在向量之上再加查询扩展与重排序，解决"找得准且省 75–95% 调用成本"——前者是地基，后者是精排省钱的楼层。
- 支持 FTS5 全文检索（零模型开箱即用）+ **SQLite 存标签路径与向量索引、Markdown 文件存实际内容**：AI 通过索引秒级定位,同事也能直接打开文件阅读,一套数据两用、不为人和 AI 分别维护两套文档。
- Rust 开发，部署后空闲时仅占用约 2MiB 内存，对话运行时 AI CLI 占用约 700MiB 内存。
- 记忆功能：目前已有会话保存，提炼短期/中期/长期记忆及遗忘记忆功能待开发。
